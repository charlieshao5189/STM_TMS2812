...
u16 soft_cnt = 0;
vu16 out_fqc = 1000;                  //输出频率，默认初始为10HZ,为输出频率的100倍，后两位为小数
vu16 out_volt = 600;
vu16 pre_out_fqc;
...
void set_parameter(void)
{
    ...
    //	volt_cmd = (((u32)out_volt*(TIMER1_ARR/2))/380);     //568=0x0238
    //
    /*最大值测试*/
    //	 out_fqc= 50;
    // 	 out_volt= 380;
    ...
    //	/* voltage command value * carrier /4 */
    //	volt_cmd = (((u32)out_volt*(TIMER1_ARR/2))/380);     // 3600= 0x0E10

//    /*skip-read valout_volte = 23040 * output frquency / sample frequency  1s内有23040 * output frquency个点，采样其中的10000个点*/
//    sin_cut = (u16)(((u32)out_fqc * 23040) / 100000);
//    /* voltage command value * carrier /4 */
//    volt_cmd = (((u32)out_volt * (TIMER1_ARR / 2)) / 3800);
	    Caculate_Control_SPWM(out_volt/38,out_fqc);//计算出正弦表
}
/*************************************************
