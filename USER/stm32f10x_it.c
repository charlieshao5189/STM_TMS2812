...
void TIM1_UP_IRQHandler(void)
{
    ...
    {
     //B相滞后A相120度
		 if((a+ N120) < N) b = a + N120;
		 else b = a + N120 - N;
		 //C相滞后B相120度
		 if((a + N240) < N) c = a + N240;
		 else c = a + N240 - N;
		 TIM1->CCR1 = SINWAVE_DATA[a];//写入比较通道1的预装载寄存器
		 TIM1->CCR2 = SINWAVE_DATA[b];//写入比较通道2的预装载寄存器
		 TIM1->CCR3 = SINWAVE_DATA[c];//写入比较通道3的预装载寄存器
		 if(a < (N-1)) a += 1;
		 else 
			a = 0;
